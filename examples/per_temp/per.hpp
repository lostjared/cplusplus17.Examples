...
namespace per {
    ...
    public:
        ...
        Per(const Per<T,D> &p, const std::string &file_n) : type{p.type}, file_name{file_n} {}
        Per(const Per<T,D> &&p, const std::string &file_n) : type{std::move(p.type)}, file_name{file_n} {}
